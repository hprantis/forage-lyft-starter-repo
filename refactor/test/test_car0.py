...
from refactor.CarFactory import CarFactory


class TestCalliope(unittest.TestCase):
    def test_battery_should_be_serviced(self):
        today = datetime.today().date()
        last_service_date = today.replace(year=today.year - 4)  # incremented for last section
        current_mileage = 0
...
class TestPalindrome(unittest.TestCase):
    ...
    # changed 3 to 1 from original test case at <---- because this model has a Spindler battery which requires service
    # every 2 years.  The test case would have been incorrect with a 3-year offset.
    def test_battery_should_not_be_serviced(self):
        today = datetime.today().date()
        last_service_date = today.replace(year=today.year - 1)  # <----
        warning_light_is_on = False
